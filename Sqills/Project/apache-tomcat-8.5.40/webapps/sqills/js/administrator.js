$(function () {
	...
	(function () {
		...
		$("#editReservation").click(function () {
			...
			date = $("#date2").val();
			reservation = {
					eventId: $("#dt-select").DataTable().row().data().eventId,
					room: {
		...
		$("#editButton").click(function () {
			...
			$.getJSON("/sqills/api/rooms", function (data) {
				...
				$("#email2").val(reservation.user.email).change();
				$("#name2").val(reservation.name).change();
				$("#visible2").prop("checked", reservation.visible).change();
	...
	(function () {
		$("#btnYes").click(function () {
			var row;
			row = $("#dt-select").DataTable().row(".selected");
			$.ajax("/sqills/administrator/reservations?eventId=" + row.data().eventId, {
				method: "DELETE",
				success: function () {
					row.deselect();
					row.remove().draw();
