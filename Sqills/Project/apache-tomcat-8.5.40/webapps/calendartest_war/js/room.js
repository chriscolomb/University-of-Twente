$(function () {
	...
	(function () {
		...
		clock = $("#clock");
		room = $("html").data("room");
		url = "/sqills/api/rooms/" + room + "/reservations";
		...
		(function loop() {
			...
			$.getJSON(url, function (data) {
				...
				toIntervalString = function (startTime = new Date().getTime(), endTime = new Date().getTime()) {
					return new Date(startTime).toString().substring(16, 21) + " - " +
							new Date(endTime).toString().substring(16, 21);
				};
	...
	(function () {
		$(".open").click(function () {
			...
			arrow = $("#arrow");
			arrow.rotate({
				animateTo: function (rotation = arrow.getRotateAngle()[0]) {
					return rotation === undefined ? 180 : (rotation + 180) % 360;
				}()
			})
		});
	})();
